...
function renderSetup() {
  ...
  for (var i = 0; i < GOOD_ROLES.length; i++) {
    ...
    h += '<label class="' + (checked ? 'checked' : '') + '" onclick="toggleRole(\'' + r + '\')"><span class="faction-label good">好人方</span><span class="role-name">' + r + '</span></label>';
  }
  h += '</div>';
  ...
  for (var i = 0; i < EVIL_ROLES.length; i++) {
    var r = EVIL_ROLES[i];
    var checked = state.activeRoles.indexOf(r) !== -1;
    h += '<label class="' + (checked ? 'checked' : '') + '" onclick="toggleRole(\'' + r + '\')"><span class="faction-label evil">反方</span><span class="role-name">' + r + '</span></label>';
  }
  h += '</div></div>';
