...
/* ===== Tools Page - Statistics Panel ===== */
function toggleStatsSection(id) {
  var body = document.getElementById(id);
  var arrow = document.querySelector('[data-target="' + id + '"] .stats-arrow');
  if (!body) return;
...
function renderStatsPanel() {
  ...
  if (!history || history.length === 0) {
    el.innerHTML = '<div class="stats-empty">暂无对局记录</div>';
    return;
  }

  var GOOD = ['梅林','派西维尔','忠臣','兰斯洛特(蓝)'];
  var EVIL = ['莫甘娜','刺客','莫德雷德','奥伯伦','爪牙','兰斯洛特(红)'];
  var DISPLAY_MAP = {'忠臣':'忠诚仆人','兰斯洛特(蓝)':'蓝兰斯洛特','兰斯洛特(红)':'红兰斯洛特'};
  var DISPLAY_ROLES = ['梅林','派西维尔','忠诚仆人','蓝兰斯洛特','莫甘娜','刺客','莫德雷德','奥伯伦','红兰斯洛特'];
  var DATA_ROLE = {'忠诚仆人':'忠臣','蓝兰斯洛特':'兰斯洛特(蓝)','红兰斯洛特':'兰斯洛特(红)',
                   '梅林':'梅林','派西维尔':'派西维尔','莫甘娜':'莫甘娜','刺客':'刺客','莫德雷德':'莫德雷德','奥伯伦':'奥伯伦'};

  // ——— 1. 阵营胜率 ———
  var goodWins = 0, evilWins = 0;
  for (var i = 0; i < history.length; i++) {
    if (history[i].winner === 'good') goodWins++;
    else if (history[i].winner === 'evil') evilWins++;
  }
  var total = goodWins + evilWins;
  var goodPct = total > 0 ? (goodWins / total * 100).toFixed(1) : '0.0';
  var evilPct = total > 0 ? (evilWins / total * 100).toFixed(1) : '0.0';

  // ——— 2. 角色胜率 ———
  var roleStats = {};
  for (var ri = 0; ri < DISPLAY_ROLES.length; ri++) {
    roleStats[DISPLAY_ROLES[ri]] = {total: 0, wins: 0};
  }
  for (var i = 0; i < history.length; i++) {
    var rec = history[i];
    if (!rec.identities) continue;
    var isGoodWin = rec.winner === 'good';
    var isEvilWin = rec.winner === 'evil';
    for (var j = 0; j < rec.identities.length; j++) {
      var role = rec.identities[j].role;
      if (!role) continue;
      var displayRole = DISPLAY_MAP[role] || role;
      if (roleStats[displayRole]) {
        roleStats[displayRole].total++;
        if ((GOOD.indexOf(role) !== -1 && isGoodWin) || (EVIL.indexOf(role) !== -1 && isEvilWin)) {
          roleStats[displayRole].wins++;
        }
      }
    }
  }
  ...
  var roleSorted = DISPLAY_ROLES.slice().filter(function(r) { return roleStats[r].total > 0; });
  roleSorted.sort(function(a, b) {
    var ra = roleStats[a].total > 0 ? roleStats[a].wins / roleStats[a].total : 0;
    var rb = roleStats[b].total > 0 ? roleStats[b].wins / roleStats[b].total : 0;
    return rb - ra;
  });

  // ——— 3. 刺客命中率 ———
  var assassinTotal = 0, assassinHits = 0;
  for (var i = 0; i < history.length; i++) {
    if (history[i].assassinTarget) {
      assassinTotal++;
      if (history[i].assassinSuccess) assassinHits++;
    }
  }
  ...
  for (var i = 0; i < history.length; i++) {
    var rec = history[i];
    if (!rec.identities || rec.identities.length < 2) continue;
    var goodPlayers = [], evilPlayers = [];
    for (var j = 0; j < rec.identities.length; j++) {
      var id = rec.identities[j];
      if (GOOD.indexOf(id.role) !== -1) goodPlayers.push(id.name);
      else if (EVIL.indexOf(id.role) !== -1) evilPlayers.push(id.name);
    }
    var isGoodWin = rec.winner === 'good';
    var isEvilWin = rec.winner === 'evil';
    function processPair(arr, won) {
      for (var a = 0; a < arr.length; a++) {
        for (var b = a + 1; b < arr.length; b++) {
          var p1 = arr[a], p2 = arr[b];
          var key = p1 < p2 ? p1 + '|' + p2 : p2 + '|' + p1;
          if (!teammateMap[key]) teammateMap[key] = {p1: p1, p2: p2, total: 0, wins: 0};
          teammateMap[key].total++;
          if (won) teammateMap[key].wins++;
        }
      }
    }
    processPair(goodPlayers, isGoodWin);
    processPair(evilPlayers, isEvilWin);
  }

  var teammateList = [];
  for (var key in teammateMap) {
    if (teammateMap[key].total >= 2) {
      teammateList.push(teammateMap[key]);
    }
  }
  teammateList.sort(function(a, b) {
    var ra = a.total > 0 ? a.wins / a.total : 0;
    var rb = b.total > 0 ? b.wins / b.total : 0;
    return rb - ra;
  });
  ...
  h += '<div class="stats-section-header" onclick="toggleStatsSection(\'stats-camp-body\')" data-target="stats-camp-body">';
  h += '<span class="stats-title">阵营胜率</span>';
  h += '<span><span class="stats-summary">好人 ' + goodPct + '%</span><span class="stats-arrow">&#9660;</span></span>';
  h += '</div>';
  h += '<div class="stats-section-body" id="stats-camp-body">';
  h += '<div class="stats-section-body-inner">';
  h += '<div class="stats-summary-line">好人方：<span class="good">' + goodWins + '场 (' + goodPct + '%)</span></div>';
  h += '<div class="stats-summary-line">坏人方：<span class="evil">' + evilWins + '场 (' + evilPct + '%)</span></div>';
  h += '</div></div></div>';

  // Section 2: 角色胜率
  var roleSummary = roleSorted.length > 0 ? (function(){
    var best = roleSorted[0];
    var pct = roleStats[best].total > 0 ? (roleStats[best].wins / roleStats[best].total * 100).toFixed(1) : '0.0';
    return best + ' ' + pct + '%';
  })() : '暂无数据';

  h += '<div class="stats-section">';
  h += '<div class="stats-section-header" onclick="toggleStatsSection(\'stats-role-body\')" data-target="stats-role-body">';
  h += '<span class="stats-title">角色胜率</span>';
  h += '<span><span class="stats-summary">' + roleSummary + '</span><span class="stats-arrow">&#9660;</span></span>';
  h += '</div>';
  h += '<div class="stats-section-body" id="stats-role-body">';
  h += '<div class="stats-section-body-inner">';
  if (roleSorted.length > 0) {
    h += '<table class="stats-table"><thead><tr><th class="rank-col">#</th><th>角色</th><th>场次</th><th>胜场</th><th>胜率</th></tr></thead><tbody>';
    for (var ri = 0; ri < roleSorted.length; ri++) {
      var r = roleSorted[ri];
      var st = roleStats[r];
      var pct = st.total > 0 ? (st.wins / st.total * 100).toFixed(1) + '%' : '0.0%';
      var isGoodRole = GOOD.indexOf(DATA_ROLE[r] || r) !== -1;
      h += '<tr><td class="rank-col">' + (ri + 1) + '</td><td class="' + (isGoodRole ? 'stats-good' : 'stats-evil') + '">' + r + '</td><td>' + st.total + '</td><td>' + st.wins + '</td><td>' + pct + '</td></tr>';
    }
    h += '</tbody></table>';
  } else {
  ...
  // Section 3: 刺客命中率
  var assassinSummary = assassinTotal > 0 ? '刺杀' + assassinTotal + '次' : '暂无数据';
  h += '<div class="stats-section">';
  h += '<div class="stats-section-header" onclick="toggleStatsSection(\'stats-assassin-body\')" data-target="stats-assassin-body">';
  h += '<span class="stats-title">刺客命中率</span>';
  h += '<span><span class="stats-summary">' + assassinSummary + '</span><span class="stats-arrow">&#9660;</span></span>';
  h += '</div>';
  h += '<div class="stats-section-body" id="stats-assassin-body">';
  h += '<div class="stats-section-body-inner">';
  if (assassinTotal > 0) {
    var assassinPct = (assassinHits / assassinTotal * 100).toFixed(1);
    h += '<div class="stats-summary-line">共刺杀 <strong>' + assassinTotal + '</strong> 次，命中 <strong>' + assassinHits + '</strong> 次，命中率 <strong>' + assassinPct + '%</strong></div>';
  } else {
    h += '<div class="stats-empty">暂无数据</div>';
  }
  h += '</div></div></div>';
  ...
  // Section 4: 队友胜率
  var teammateSummary = teammateList.length > 0 ? teammateList.length + '组' : '暂无数据';
  h += '<div class="stats-section">';
  h += '<div class="stats-section-header" onclick="toggleStatsSection(\'stats-teammate-body\')" data-target="stats-teammate-body">';
  h += '<span class="stats-title">队友胜率</span>';
  h += '<span><span class="stats-summary">' + teammateSummary + '</span><span class="stats-arrow">&#9660;</span></span>';
  h += '</div>';
  h += '<div class="stats-section-body" id="stats-teammate-body">';
  h += '<div class="stats-section-body-inner">';
  if (teammateList.length > 0) {
    for (var ti = 0; ti < teammateList.length; ti++) {
      var t = teammateList[ti];
      var pct = (t.wins / t.total * 100).toFixed(1);
      h += '<div class="stats-team-item"><span class="stats-team-name">' + t.p1 + ' & ' + t.p2 + '</span><span class="stats-team-detail">同队' + t.total + '场 胜' + t.wins + '场 (<span class="rate">' + pct + '%</span>)</span></div>';
    }
  } else {
    h += '<div class="stats-empty">暂无数据</div>';
  }
